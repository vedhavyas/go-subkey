...
func TestDerive(t *testing.T) {
	testsMap := map[Scheme][]struct {
		...
		ed25519.Scheme{}: {
			...
			{
				uri:       "crowd swamp sniff machine grid pretty client emotion banana cricket flush soap//foo",
				secret:    "0x833f823fbc06b721890c56ecb5dc3972039b2e84bb8b6776e801d95e5dcdd18d",
			...
			},
			{
				...
				secret:    "0x21346646d89dfcf14d69152583ccd30f3ebc385f0b112c54b477be16ff4fcfb9",
	...
	for scheme, tests := range testsMap {
		for _, c := range tests {
			...
			if c.secret != "" {
				sec := s.Secret()
				assert.Equal(t, c.secret, common.EncodeHex(sec))
			}
			pub := s.Public()
